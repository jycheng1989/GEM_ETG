...
subroutine init
  ...
  beta=betai !yjhu, beta is now set in gem_equil.f90
  br0 = rmaj0
  lr0 = r0
  qp = q0p
  lz = pi2*q0abs*rmaj0
  delz = lz/ntheta
  rneu = nuacs
  kxcut=pi2/lx*64.1 !yjhu
  kycut=pi2/ly*16.1
  if(onemd==1)then
!yjhu     kxcut=pi2*shat0*pi2/ly*2.1
     kxcut=pi2/lx*1.1 !yjhu
     kycut=pi2/ly*1.1
  end if
  achii=0.
  achie=0.
  ...
  if(myid.eq.master)then
     ...
     write(19,*)'i,   sf,   ni,   ne,   ti,   te,   capni,   capne, capti,    capte'
     do i = 0,nr
...
!  upa0 = 0.
  ...
  if(myid.eq.master)then
     ...
     write(9,*) 'lr(1),lr(2),lr_e=', lr(1),lr(2),lr_e
     write(9,*) 'mimis(1:2),q(1:2) = ', mims(1),mims(2),q(1),q(2)
     close(9)
  end if
  ...
  if(myid.eq.master)then
     ...
     write(*,*) 'lr(1),lr(2),lr_e=', lr(1),lr(2),lr_e     
!yjhu     write(*,*) 'f0p,psip(nr/2),Bunit,candyf0p = ',f0p,psip(nr/2),bunit,candyf0p
...
subroutine cpush(n,ns)
  use mpi
  use gem_com, only : mm, x3,y3,z3,u3, x2,y2,z2,u2, w2,w3, mu, xii, z0i, pzi, eki, u0i, & !intent(inout)
       & efl_es, efl_em, pfl_es, pfl_em,  & !store diagnostic information
       & lx, ly, lz, dx, dy, dz, dt, gclr, ierr, iflr, iorb, ipara, kcnt, lr0, lr, &
       & vexbsw, vparsw, vwidth, q, mims, ex, ey, ez, delbx, delby, dpdz, dadz, apar, &
       & avewi, nos, ke, nsubd, peritr, pi, tor, totvol, nonlin, tmm, vol, br0, myid,ran2,iseed,pi2
...
!  call joule
  ...
  if(myid.eq.master)then
     ...
     write(*,10)tdum,rmsphi(n),rmsapa(n),pf,efe,pfi,efi,avewi(1,n),&
          avewe(n),yyre(1,0),yyim(1,0),yyamp(1,0)

10   format(1x,f8.2,15(2x,e10.3))
11   format(6x,5(2x,e12.5))
12   format(1x,f8.2,12(2x,e12.5))
13   format(1x,i6,4(2x,e10.3),2x,i7,2x,i7)
15   format(1x,i6,8(2x,e10.3))

     write(9,10)tdum,rmsphi(n),rmsapa(n),pf,efe,pfi,efi,avewi(1,n),&
          avewe(n),yyre(1,0),yyim(1,0),yyamp(1,0)
...
subroutine yveck1(u,n)
  ...
  use gem_equil
  use gem_fft_wrapper
  use evolution_mod, only:write_evolution_file !yjhu
  implicit none
  !     
  real :: u(0:imx,0:jmx,0:1)
  INTEGER :: n,i,j,k
  ...
  call MPI_BARRIER(MPI_COMM_WORLD,ierr)
!call write_evolution_file(tmp3d) !yjhu added
end subroutine yveck1

!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

real function ran2(idum)
integer :: idum, im1, im2, imm1, ia1, ia2, iq1, iq2, ir1, ir2, ntab, ndiv !yjhu added
real    :: am, eps, rnmx !yjhu added
...
!!$        xt=mod(xt+800.*lx,lx)       !   BOUNDARY
!!$        yt=mod(yt+800.*ly,ly)       !   BOUNDARY
!!$        xt = min(xt,lx-1.0e-8)      !   BOUNDARY
!!$        yt = min(yt,ly-1.0e-8)      !   BOUNDARY
        xt=modulo(xt,lx)       !   BOUNDARY
        yt=modulo(yt,ly)       !   BOUNDARY

        !yjhu     include 'ppushlie.h'
        i=int(xt/dx)
        j=int(yt/dy)
        ...
        x110=wx1*wy1*wz0
        x111=wx1*wy1*wz1
        !yjhu exp1 = x000*ex(i,j,k) + x100*ex(i+1,j,k)  &
        exp1 = exp1+x000*ex(i,j,k) + x100*ex(i+1,j,k)  &
             + x010*ex(i,j+1,k) + x110*ex(i+1,j+1,k) + &
             x001*ex(i,j,k+1) + x101*ex(i+1,j,k+1) +   &
             x011*ex(i,j+1,k+1) + x111*ex(i+1,j+1,k+1)

        !yjhu eyp = x000*ey(i,j,k) + x100*ey(i+1,j,k)  &
        eyp = eyp + x000*ey(i,j,k) + x100*ey(i+1,j,k)  &
             + x010*ey(i,j+1,k) + x110*ey(i+1,j+1,k) + &
             x001*ey(i,j,k+1) + x101*ey(i+1,j,k+1) +   &
             x011*ey(i,j+1,k+1) + x111*ey(i+1,j+1,k+1)

        !yjhu ezp = x000*ez(i,j,k) + x100*ez(i+1,j,k)  &
        ezp = ezp + x000*ez(i,j,k) + x100*ez(i+1,j,k)  &
        ...
        delbxp = delbxp + x000*delbx(i,j,k) + x100*delbx(i+1,j,k)  &
        ...
        !yjhu delbyp = x000*delby(i,j,k) + x100*delby(i+1,j,k)  &
        delbyp = delbyp + x000*delby(i,j,k) + x100*delby(i+1,j,k)  &
...
!yjhu     nue=rneu*(wx0*nue0(i)+wx1*nue0(i+1))*(wx0*zeff(i)+wx1*zeff(i+1)+hee)
     nue=0.0 !yjhu
...
!!$     y3e(m) = min(y3e(m),ly-1.0e-8)
     y3e(m)=modulo(y3e(m)-laps*2*pi*qr*lr0/q0*sign(1.0,q0),ly)
...
!!$        yt = min(yt,ly-1.0e-8)      !   BOUNDARY
        ...
        !yjhu delbxp = x000*delbx(i,j,k) + x100*delbx(i+1,j,k)  &
        delbxp = delbxp + x000*delbx(i,j,k) + x100*delbx(i+1,j,k)  &
             + x010*delbx(i,j+1,k) + x110*delbx(i+1,j+1,k) + &
             x001*delbx(i,j,k+1) + x101*delbx(i+1,j,k+1) +   &
             x011*delbx(i,j+1,k+1) + x111*delbx(i+1,j+1,k+1)
...
!yjhu     nue=rneu*(wx0*nue0(i)+wx1*nue0(i+1))*(wx0*zeff(i)+wx1*zeff(i+1)+hee)
     ...
     if(abs(w3e(m)).gt.0.1.and.nonline==1)then
        w3e(m) = 0.
        w2e(m) = 0.
        mynowe = mynowe+1
     end if
...
subroutine poisson(n,ip)
  ...
  do it = 1,iter
!yjhu     call den0_phi(ip,n,it) !cancellation scheme is not necessary
...
!        if(i==iter)call jpar0(ip,n,i,1)
  ...
  if(ip==1)jpred = 1
  if(ip==0)jcorr = 1
!  if(iter>1 .and. rma(iter)/rma(iter-1)>1.1)then
!     apar(:,:,:) = 0.
!     if(ip==1)jpred = 0
!     if(ip==0)jcorr = 0
!     rmsapa(n) = 0.
...
13 format(1x,i6,7(2x,i7))
  if(myid.eq.master)then
     ...
     write(16,13)n,ipred,icorr,jpred,jcorr,noen,nowe
     close(16)
  end if
...
subroutine jpar0(ip,n,it,itp)
  ...
  use gem_com, only : apar, phi, dene, amie, br0, dx, dy, dz, jac, &
  ...
  !      upa0(:,:,:) = apar(ip,:,:,:)
  !      return
  if(it.eq.1)then !first iteration
     apar(:,:,:) = 0.
     upa0(:,:,:) = 0.
     upa00(:,:,:) = 0.
     den0apa(:,:,:) = 0.
...
!!$        wght1 = wght1*aparp*vpar*vpar/amie/ter*xnp 
        wght1 = wght1*aparp*vpar*vpar/amie/ter*xnp/lr_e !yjhu
...
!!$        myupa0(i+1,j+1,k+1)=myupa0(i+1,j+1,k+1)+wght1*w111(m)
        do l=1, lr_e !yjhu
           ...
           x110=wx1*wy1*wz0
           x111=wx1*wy1*wz1
           myupa0(i,j,k)      =myupa0(i,j,k)      +wght1*x000
           myupa0(i+1,j,k)    =myupa0(i+1,j,k)    +wght1*x100
           myupa0(i,j+1,k)    =myupa0(i,j+1,k)    +wght1*x010
           myupa0(i+1,j+1,k)  =myupa0(i+1,j+1,k)  +wght1*x110
           myupa0(i,j,k+1)    =myupa0(i,j,k+1)    +wght1*x001
           myupa0(i+1,j,k+1)  =myupa0(i+1,j,k+1)  +wght1*x101
           myupa0(i,j+1,k+1)  =myupa0(i,j+1,k+1)  +wght1*x011
           myupa0(i+1,j+1,k+1)=myupa0(i+1,j+1,k+1)+wght1*x111
        enddo
     end if
     if(itp==1)then
!!$           wght0 = wght0*aparp*vpar*zdot/amie/ter*xnp 
        wght0 = wght0*aparp*vpar*zdot/amie/ter*xnp/lr_e
...
!!$           myupa(i+1,j+1,k+1)=myupa(i+1,j+1,k+1)+wght0*w111(m)
        do l=1, lr_e !yjhu
           ...
           x110=wx1*wy1*wz0
           x111=wx1*wy1*wz1
           myupa(i,j,k)      =myupa(i,j,k)      +wght0*x000
           myupa(i+1,j,k)    =myupa(i+1,j,k)    +wght0*x100
           myupa(i,j+1,k)    =myupa(i,j+1,k)    +wght0*x010
           myupa(i+1,j+1,k)  =myupa(i+1,j+1,k)  +wght0*x110
           myupa(i,j,k+1)    =myupa(i,j,k+1)    +wght0*x001
           myupa(i+1,j,k+1)  =myupa(i+1,j,k+1)  +wght0*x101
           myupa(i,j+1,k+1)  =myupa(i,j+1,k+1)  +wght0*x011
           myupa(i+1,j+1,k+1)=myupa(i+1,j+1,k+1)+wght0*x111
        enddo
...
!yjhu  if(itp==1)call enforce(myden0(:,:,:))
     ...
     do  i=0,im
        do  j=0,jm
           do  k=0,mykm
              ...
              !yjhu den0apa(i,j,k)= myden0(i,j,k)/n0e/jac(i,k)*pidum*cn0e
           end do
        end do
     end do
  end if
...
!yjhu  real    :: u(0:imx-1,0:jmx-1,0:1)
  real, intent(in)    :: u(0:imx-1,0:jmx-1,0:1)
...
!              +2.*amp*sin(5*pi2/ly*y2e(m))*exp(-(z2e(m)-lz/2)**2/(lz/8)**2) &
!              +2.*amp*sin(6*pi2/ly*y2e(m))*exp(-(z2e(m)-lz/2)**2/(lz/8)**2) &
!              +2.*amp*sin(7*pi2/ly*y2e(m))*exp(-(z2e(m)-lz/2)**2/(lz/8)**2) &
!              +2.*amp*sin(8*pi2/ly*y2e(m))*exp(-(z2e(m)-lz/2)**2/(lz/8)**2)
        myavgw=myavgw+w2e(m)
     end if
  enddo
170 continue
...
subroutine gkpsL(nstep,ip)   
  ...
  real :: b_e, gam0_e, gam1_e !yjhu   
  real :: b2,delyz,th,bf,gamphi,kperp2
     ...
     do l=0,im-1
        do m=0,jm-1
           do n = 0,km
              ...
              b2=b2**c4*(1-onemd)
              kperp2=kx*kx*grp**2 + ky*ky*(dydrp**2*grp**2+(lr0/q0*qhatp*gthp)**2 &
                   +2*dydrp*lr0/q0*qhatp*grdgtp+(lr0/q0/radiusp)**2)  &
                   +2.*kx*ky*gxdgyp
              ...
              enddo
              b_e=emass*(kx*kx*grp**2 + ky*ky*(dydrp**2*grp**2+(lr0/q0*qhatp*gthp)**2 &
                   +2*dydrp*lr0/q0*qhatp*grdgtp)+2.*kx*ky*gxdgyp)/(bf*bf) &
                   *t0e(1)/(qel*qel) !yjhu, for electron                                                                                                                                      
              call srcbes(b_e,gam0_e,gam1_e) !yjhu, for electron        
              !   formfactor in gkps
              ...
              if(b(1).lt.3.e-5)then
                 ...
                 gamphi = 0.
                 do ns = 1, nsm
                    gamphi = gamphi + q(ns)*q(ns)*cn0s(ns)*gn0s(ns,l)/ter(ns)*(1-gam0(ns)) !yjhu ion polarization density
                 enddo
...
!yjhu  myden = rho+den0apa+fradi*(phi/ntube*xn0e(nr/2)*cn0e/t0e(nr/2)-den0)
  myden = rho !yjhu, remove cancellation scheme, which seems to be not necessary
